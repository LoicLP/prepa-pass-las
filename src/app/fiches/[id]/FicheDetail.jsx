  ...
  const handleDownloadPDF = useCallback(async () => {
    try {
      ...
      const pdfHexColors = {
        ...
        primary: { bg: '#eef2ff', accent: '#4f46e5', accentDark: '#3730a3', accentDeep: '#1e1b4b', badge: '#e0e7ff', badgeText: '#3730a3', light: '#c7d2fe', lighter: '#e0e7ff' },
              ...
              <div className="mt-6 bg-gradient-to-r from-primary-600 to-violet-600 rounded-2xl p-6 text-white">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-primary-200 text-sm mt-0.5">QCM ciblés sur « {fiche.title} », niveau PASS/LAS.</p>
                  </div>
                  <Link href={`/qcm?fiche=${fiche.id}`} className="shrink-0 px-5 py-2.5 bg-white text-primary-700 text-sm font-bold rounded-xl hover:bg-primary-50 transition-colors shadow-sm">
                    Faire un QCM →
                  </Link>
                </div>
              </div>
