...
function renderDashboard() {
  ...
  app.innerHTML = `
    ...
    <div class="border-b border-gray-200 mb-6">
      <nav class="flex gap-1 -mb-px overflow-x-auto" id="dash-tabs">
        ${renderTab('qcm', 'QCM', false)}
        ${renderTab('examens', 'Examens', false)}
        ${renderTab('stats', 'Statistiques', false)}
        ${renderTab('progression', 'Progression', true)}
        ${renderTab('classement', 'Classement', true)}
      </nav>
    </div>
  ...
  document.querySelectorAll('[data-dash-tab]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('[data-dash-tab]').forEach(b => {
        b.classList.remove('active', 'text-primary-600', 'border-primary-600');
        b.classList.add('text-gray-500', 'border-transparent');
      });
      btn.classList.add('active', 'text-primary-600', 'border-primary-600');
...
/* ========== TAB BUTTON ========== */
function renderTab(id, label, premium) {
  const isActive = dashActiveTab === id;
  const activeClass = isActive ? 'text-primary-600 border-primary-600' : 'text-gray-500 border-transparent';
  const lock = premium && !isPremium() ? '<svg class="w-3.5 h-3.5 text-gray-400 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z"/></svg>' : '';
  return `<button data-dash-tab="${id}" class="dash-tab flex items-center gap-1 px-4 py-3 text-sm font-semibold border-b-2 whitespace-nowrap ${activeClass}">${label}${lock}</button>`;
}
